"""Ponto de entrada da Codemancer Toolbox (menu + atalhos por argumentos)."""
...
def executar_por_argv(argv: list[str]) -> bool:
    ...
    try:
        ...
        elif cmd == "media" and len(argv) >= 3:
            nums = [float(x) for x in argv[2:]]
            print(calc.media(*nums))
        elif cmd == "slug" and len(argv) >= 3:
            txt = " ".join(argv[2:])
            print(su.slugificar(txt))
        else:
            return False
    except ValueError:
